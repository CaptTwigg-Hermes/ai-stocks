...
public static class ExhibitionPromptBuilder
{
    ...
    public static string RetryAfterRejectedEvidence(string originalPrompt, string rejectedHost) => originalPrompt + $$"""

        EVIDENCE CORRECTION: Independent verification rejected evidence from {{rejectedHost}}. Research again and use a different source host with an explicit publication timestamp and an exact sentence returned by the bounded fetch tool. This is your single corrective retry. Return only the strict JSON object. Do not reuse the rejected host, invent evidence, or force a trade.
        """;
    ...
    public static string Build(AgentDefinition agent, string runId, string instrumentsJson, string progressJson)
    {
        ...
        return $$"""
            ...
            You may research only public HTTPS web sources. Before deciding, you MUST use web_search to investigate at least eight currently observed issuers (or every issuer when fewer than eight are shown). Search each issuer by its full name together with terms such as financial results, guidance, corporate action, material announcement, valuation, or outlook and the current year. A search-result snippet is discovery only, not evidence. For promising results, MUST use mcp_research_fetch_public_https_tool to inspect the bounded source representation and publication metadata. When structured_article_text is present, use an exact sentence from that fetched NewsArticle/Article/PressRelease metadata as evidence because the verifier independently re-fetches those fields even when browser CSS is external. Prefer issuer investor-relations, regulatory-news, or syndicated press-release pages with an explicit publication timestamp and exact visible text. If one fetched source is unavailable or lacks independently verifiable text, search for the same event on a different source host. Do not claim that research is unavailable without first calling web_search and the fetch tool. Do not seek rival state. The only portfolio context below is your own.
