...
public static class ExhibitionPromptBuilder
{
    ...
    public static string Build(AgentDefinition agent, string runId, string instrumentsJson, string progressJson)
    {
        ...
        return $$"""
            ...
            You may research only public HTTPS web sources. Before deciding, you MUST use web_search to investigate at least three currently observed issuers (or every issuer when fewer than three are shown). Search each issuer by its full name together with terms such as financial results, guidance, corporate action, or material announcement and the current year. A search-result snippet is discovery only, not evidence. Prefer issuer investor-relations, regulatory-news, or syndicated press-release pages with an explicit publication timestamp and exact visible text. Do not claim that research is unavailable without first calling web_search. Do not seek rival state. The only portfolio context below is your own.
