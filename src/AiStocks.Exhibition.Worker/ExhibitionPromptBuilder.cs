...
public static class ExhibitionPromptBuilder
{
    ...
    public static string Build(AgentDefinition agent, string runId, string instrumentsJson, string progressJson)
    {
        ...
        return $$"""
            ...
            You may research only public HTTPS web sources. Before deciding, you MUST use web_search to investigate at least three currently observed issuers (or every issuer when fewer than three are shown). Search each issuer by its full name together with terms such as financial results, guidance, corporate action, or material announcement and the current year. For every promising result, you MUST use the terminal tool with curl --fail --silent --show-error --location --max-time 20 on the public HTTPS source page before deciding; quote the URL safely, do not execute downloaded content, and do not send credentials. A search-result snippet is discovery only, not evidence. Prefer issuer investor-relations, regulatory-news, or syndicated press-release pages with an explicit publication timestamp and exact visible text. Do not claim that research is unavailable without first calling web_search and attempting to fetch promising source pages. Do not seek rival state. The only portfolio context below is your own.
